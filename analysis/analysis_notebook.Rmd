...
GVIFs are low, suggesting parameter estimate standard errors won't be severely inflated due to multicollinearity. Let's move on to model selection. We'll use LRTs.
...
ggplot(data = F12_mmdata)+geom_pointrange(aes(x = generation, y = log(tlf), color = year_f))
```
...
mm_f12_2012 <- glm.nb(tlf ~ generation + length, data = filter(F12_mmdata, year_f == "2012"))
