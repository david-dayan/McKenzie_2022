...
(3)  If you're just looking to run a section and are feeling confident, the finalized environment is available Feb17_2023.Rdata. You can load this file and use the resulting objects, but keep in mind that some objects change throughout the notebook, so running a code chunk using the final environment may produce different results than originally or throw some errors. Use caution.   
...
Throughout this report (and previous reports), it has been useful to split the dataset up and present results by potential offspring and candidate parents. Let's keep with this approach and present the numbers of potential parents ( __table N1__ ) and the numbers of potential offspring __table N7__ .
...
Candidate parents are defined as any salmon released, or otherwise sampled, above the dam.
...
(2) __DownstreamRecycling__: All individuals at the Cougar Trap, regardless of date, are first released downstream of the dam (either in Forest Glen, or just the dam tailrace). If they return a secodn time they are released above the dam.

__LDSR / Downstream Recycling Summary__
During 2010, 2011 and 2012, neither LSDR or downstream recycling was used. All fish that were trapped and survived were released above the dam.   
...
kable(full_data_1.0 %>%
  filter(type == "cougar_trap", origin == "NOR", year > 2014) %>%
    group_by(sample_id) %>% 
    slice_max(date, n = 2, with_ties = FALSE) %>% # a couple individuals are recorded three times, this will break these counts, just get the first two instances
    ungroup() %>%
  count(year, recapture) %>%
  pivot_wider(id_cols = year, names_from = recapture, values_from = n) %>%
...
Now let's summarise the potential offspring. We define potential offspring as any salmon sampled that could have been produced above the dam. Therefore, EVERY NOR salmon that is sampled is considered a potential offspring.

__An important definition__: NOR salmon collected at the trap or during SGSs may be produced above the dam. Alternatively they may be produced below the dam or elsewhere. We define __NOR immigrants__ as NORs that do not assign to at least one parent above the dam, and infer that NOR immigrants are produced below the dam or elsewhere. 

Once again we present the counts from the final filtered dataset, because these are the values we use to calculate demographic results such as CRR, and there may be duplicates in the unfiltered dataset due to batch sampling or carcass sampling.
...
Here we summarise assignment of offspring to parents, infer age structure, evaluate the efficacy of LSDR and recycling programs at excluding NOR immigrants from above the dam, and conduct some modeling to understand the relationship between time, sex and the proportion of NOR immigrants vs NORs produced above the dam at the trap. 
...
People may be interested in just how many individuals show up at the trap, not ALL potential offspring, this will also be useful for comparing our assignment rates with the previously published ones (which don't include carcass samples). We will consider this in greater detail in the section [Recycling], but let's make revised table split by source.
...
__Big Change__  
The offspring year perspective was causing some mis-interpretation among readers. So I also conducted a AAM analysis from the perspective of offspring of each parental cohort. This can be found [here]{#AAM_link}. 
...
An additional note here: given previous findings that release day effects vary across years (e.g. temporally variable selection), I tried to fit a random slopes model allowing the relationship between jday and TLF to vary across years, but this had convergence challenges when using fully saturated fixed effects structure. When I used what wound up as the final model's fixed effects structure and then refit by reml two models (the final and the final with random slope for jday|year), I found the LRT andAIC did not support the more complex random effect structure but BIC did. 

```{r}
ggplot(data = mm_data)+geom_smooth(aes(x = jday_c, y = tlf, color = year), method = "glm.nb")
```
...
Another option here is to resample the NOR immigrants to match the timing distribution of the F1s. this would be a good supplemental analysis.  
...
One issue with this analysis is that NOR immigrants are known to differ from NOR descendants by arrival timing. Arrival time itsef might directly affect TLF. In this case, including arrival/release date as a covariate allows us to control for this effect. However, what if there is a relationship between fitness and the tendency to arrive later as an NOR immigrant. For example, immigrants disperse farther when they face lower reproductive success in the face of competition. 

We can control for this problem (somewhat) by resampling the NOR immgrants to match the ditribution of arrival times in the F1s. Let's try.

```{r}
F12_mmdata %>%
  ggplot()+geom_density(aes(x = jday_c, color = generation))
```

```{r}
# get density of release dates
dens.obs = density(filter(F12_mmdata, generation == "F1")$jday_c, adjust=0.8)
...
sample(filter(F12_mmdata, generation == "NORimmigrant")$jday_c, 100, replace=FALSE, prob=predict_density(filter(F12_mmdata, generation == "NORimmigrant")$jday_c))


resample.obs = sample(filter(F12_mmdata, generation == "NORimmigrant")$jday_c, 100, replace=TRUE, prob=dens.obs$y)



F12_mmdata_date_normalized <- F12_mmdata %>%
...
## Age Variance {#AAM_link}

Both Nick and Kathleen were interested in the relationship between cohort productivty and the resulting age structure. I'm still not sure of the best way to tackle this question, but one place to start is to examine how if there is any difference is the age structure of offspring from each cohort, and if there is a shift when the pHOS goes from 100% (2007 - 2009), to around 70%.



First let's visualize the pHOS pattern

```{r}


# first let's visualize the pHOS pattern
ggplot(data = filter(parents, year < 2016)) + geom_bar(aes(x = year, fill = origin), position = "fill")+ylab("Proportion")+ggtitle("pHOS per parental cohort")+scale_fill_bright()

```

Now heres the original plot
```{r}
...
We can see the problem here (why folks are getting hung up on this plot), it's hard to tell if the age structure in a given year is driven by parental productivity or if the actual age structure changes. Let's change the perspective, what is the age structure for a given parent cohorts set of offspring?

```{r}
# this one is best for indiciating that age structure of offspring is very similar across parent years
ggplot(data = filter(aam_data, parent_year < 2016))+geom_bar(aes(fill = age, x = parent_year), position = "fill")+ylab("Proportion")+ggtitle("age structure of offspring per parental cohort")+scale_fill_viridis_d()+theme_bw()
```

Here it is easy to see that after controlling for productivity, the age structure of offspring doesn't change much across parent years. But these 100% stacked bar charts (while being easy to compare changes in age structure across years) create other problems. 
Let's use a more traditional stacked bar.


```{r}
# went with this one
ggplot(data = aam_data)+geom_bar(aes(fill = age, x = as.factor(parent_year)))+ylab("Number of Offspring")+ xlab("Year Parent Released") + scale_fill_viridis_d(name = "Age of\nOffspring")+theme_bw()+scale_x_discrete(labels = c("2007", "2008", "2009", "2010", "2011", "2012", "2013", "2014", "2015*", "2016**", "2017**"))

#but others convey different information


# this one is not so great
#ggplot(data = filter(aam_data, parent_year < 2016))+geom_bar(aes(fill = age, x = as.factor(parent_year)), position = "dodge")+ylab("Number of Offspring")+ggtitle("age structure of offspring per parental cohort")+scale_fill_viridis_d()+theme_bw()
```

Not any obvious/strong patterns in age structure of offspring from year to year. Perhaps a trend towards younger over time? Let's see if it's significant

```{r}

aam_data %<>%
  mutate(age_n = as.numeric(as.character(age)))

aam_data %>%
  filter(parent_year < 2016) %>%
  group_by(parent_year) %>%
  summarise(mean_age = mean(age_n))

summary(aov(age_n ~ parent_year, data = filter(aam_data, parent_year < 2016)))

ggplot(data = filter(aam_data, parent_year < 2016))+geom_violin(aes(x = parent_year, y = age_n, group = parent_year))+geom_smooth(aes(x = parent_year, y = age_n), method = "lm")+theme_bw()


```

Yes, a small but significant pattern. The age distribution of the offspring gets 0.014 years younger with each parent year (p = 0.015).

Let's also reframe the entire AAM results this way. LEt's recalculate averages according to parent year when we know all offspring are assigned (2007 - 2014).
```{r}
aam_data %>%
  filter(parent_year < 2015) %>%
  count(age) %>%
...
Analyses and code in this section has minor issues (like insufficient power), is messy, deals with a very minor question or poorly conceived. The most important questions addressed in this section have been addressed in the [Other Analyses](#other-analyses) section above.

The goal is to keep a log of process here, not to apply this code. Do not set code chunks to evaluate unless you know what you are doing!
