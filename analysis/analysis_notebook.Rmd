...
mm_pair_date <- glmmTMB(pair_tlf ~ cross_f + father_jday_c+ (1|year_f), data = mate_pair, family = nbinom2)

# what about OLRE?
mate_pair %<>%
  mutate(mp_f = as.factor(mate_pair))
mm_pair_OLRE <- glmmTMB(pair_tlf ~ cross_f + (1|year_f) + (1|pair), data = mate_pair, family = nbinom2)
...
ggplot(data = mate_pair_freqs_long, aes(x = parent_year, y = freqs, color = obs_exp))+geom_point()+ facet_grid(~cross)+theme(axis.text.x = element_text(angle = 90))
...
kable(f1s %>%
        ...
        pivot_wider(id_cols = offspring_year, names_from = cross, values_from = n), caption = "F1 sample sizes, by year and parent cross type") %>% kable_classic(full_width = F, html_font = "Arial" )
```
