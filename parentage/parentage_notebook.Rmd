...
"mckenzie_2022/parentage/COLONY"    
...
- Subdirectories are organized by offspring year, each subdirectory contains:  
    ...
    - note that the same parent can occur across multiple offspring years, so all information for a given parent may be spread across multiple subdirectories  
...
Previous workers updated/improved their methods between reports. There is no consistent colony approach used. As documented below there are different Colony approaches WITHIN years (though only single used in a given report/paper), as well as different approaches ACROSS years (different approaches for COLONY are combined into a single report/manuscript). Previous workers updated and improved their methods between reports.
...
_2015_  
This approach corresponds to the "long" directories instead of the run1 and run2 directories
...
I will use the the marker error rates published previously for this system. Regenotyping of individuals has confirmed that when at least 7 markers are successfully genotyped for a sample (the cutoff for incusion in the dataset), the overall average error rate is ~2% . However, we will also use the inferred pedigree to confirm this error rate (as discussed in Wang 2018 - Methods in Ecology and Evolution). Update: Error rates based on new data (but old prior) are consistent.

# Cervus Log

## Sard Approach
Similar to Colony, there is no consistent approach used for Cervus across all offspring years that can be applied. Previous workers updated and improved their methods between reports.
...
| 
| 3.  Error Rates: We will stick with the 2% error rate. This is slightly conservative, as our error rate estimated from replication is ~1.6% and we find similar estimated error rates (~2%) using the pedigree based approach of Colony.  
| 
| 4.  7 or more typed loci required for comparison (e.g. overlapping between parent and offspring), based on the non-exclusion probabilities (NEPs), and the published cutoffs used in other systems (NSNT, SSNT). We present the NEPs for each year given the 7 - 11 compared genotypes elsewhere. In short (given 1 mismatch tolerance), 7 is the point where substantial number single parentages assigned would be spurious if we tolerate a single mismatch (e.g. 6 mathcing loci). If we consider the 2010 offspring year, the type I error rate for a single parent assignment (NEP) is 0.09% which sounds small until you think about the ~350 comparisons made with this NEP, which results in about 32% of offspring spuriously assigned to a single parent. In the bigger years this cutoff (7 loci + 1 mismatch) all but guarantees a single parent will be assigned for each offspring-parent combo scored at only 7 loci. HOWEVER (and this is very important), very few individuals are scored at such a small number of loci in the dataset, and a careful analysis of the worst-case scenario reveals that (given the empirical dataset sizes, and missingness), the estimated NEPs are expected to produce ~1% type I error in the pedigree. There's a lot more analysis of this elsewhere for those curious, including some at the end of this notebook.  
...
1.	Criteria below are always applied __in the order presented__ here.   
2.	Run Colony and Cervus for each offspring year according to parameters described in "Log" sections above.  
3.	Colony pedigree:   
    a.	Mismatch Filtering (see note 2 below for some thoughts about this if continuing this work in the future):    
    ...
    b. remove any impossible trios (see note 1 below though)    
        ...
        ii.	If one parent of a trio was collected as a carcass sample, recovery date must be after the release date of the other parent    
4.	Cervus pedigree:    
...
5.	Consensus Pedigree – here we combine the colony and cervus results into the final pedigree used for later analysis  
    ...
    f.	if only one program makes assignment:  
        i.	keep assignment (assignment takes precedence to no assignment)(see note 6)



## Notes

Here we record some __known__ challenges with the approach above.
...
1.	The previous approach used for NSNT (called Bohn here for short, but represents Bohn's attempts to best replicate Evans and Blacks approaches) and for the latest USACE report (but not manuscript) in MCKR conducts a single colony and cervus run for each offspring year. This is different than the approach taken by Sard in 2010-2013, where each offspring year- parent year assignment is run separately in cervus, then the results are concatenated and assignment criteria applied to the concatenated result tables.  
    Each approach presents its own issues. For the Bohn approach, combining all parents for a given offspring year into a single cervus/colony run means that impossible parentages are considered by colony and cervus. This biases CRR downwards, because good parentage assignments are excluded in favor of impossible assignments, which are subsequently filtered out.  
    For the Sard 2010-2013 approach, we reproduce this problem, despite running cervus separately for each offspring-year parent-year pair, because parents are picked one sex at a time using the concatenated dataset, but we also introduce a new problem because ties are supposed to be broken by LOD and LOD is not comparable across Cervus runs that include different data (they're likelihoods!).  
...
2.	Everyone who worked on these projects previously seems to have only output the trios with positive LOD score from Cervus. This seems to be motivated by the massive file size (1 line per each offspring x father x mother combo – files sizes >300gb are common), but this introduces a challenge: Cervus only permits filtering the output file by the TRIO LOD score, but everyone has been interested in making single parent assignments if no good trio is available. As a consequence, __the true best single parent in cervus (according to number of mismatches or highest LOD score), may not appear in the output, because there is no trio with a positive LOD that includes this parent__. This means we don’t have the all the information we need to apply the rules we claim to apply without resorting to manual matching for every single possible parent. In other words, previous methods sections statements such as “we chose the parent with the fewest mismatches” are misleading, because we don’t actually have this information on hand. Among others, this may have been previous workers’ motivation for trusting colony assignments over cervus assignments.  
    ...
    __Update__ – my desired approach (print the entire cervus output to file) is unmanageable with the equipment and time frame that we are working with for this project. Output files get up to the terabyte range. It could be done with a cervus install on the server, but this would be very time consuming because cervus is windows dependent and would require setting up a virtual machine and a lot of other work.  
    Instead, to evaluate how severe a problem the filtered output represents. I checked if there was ever a case where colony made an assignment to a parent that didn’t have mismatch data. It never happened for the 251 offspring in 2018 for the NSNT project despite many single parent assignments. This suggests that a single good parent is usually enough to push trio LOD above 0 for at least one row in the cervus output. So, the problem still exists (we may not have all the information to select the true best single parent), but it is likely uncommon enough that we should probably move on to bigger issues.    
...
3.	For the consensus pedigree there are a few general ideas from previous papers we are trying to implement into specific criteria here.   
    ...
    c.	The power to exclude parentage is MUCH greater for trios than duos (see non-exclusion probabilities in table s3 in Sard 2016, and elsewhere)      
...
5.	Application of the ideas above (3) tend to result in increasing the number of assignments when generating a consensus pedigree relative to the pedigree produced by either colony or cervus. This is anti-conservative (biases CRR and fitness upwards), however we have also identified occasions where either the Colony of Cervus pedigrees alone may by conservative (bias CRR and fitness downward, for example the impossible parentage problem in Colony described in note 1 above, or the mismatch filtering problem in note 4). Ultimately there is no perfect way to make a consesus pedigree and we have no empirical information against which to optimize our approach. Instead we prioritize consistency across years. __Update__ Yep, suspicion here is right, Cervus produces a pedigree with 1494 at least single parent assignemnts, and Colony produces a pedigree with 1480, but the consensus pedigree has 1669.  

6.   This is probably where most type I error from individuals with a lot of missing data accrues. We establish elsewhere it's probably not a big deal (~1% of final assignments) given how complete the dataset is, but if anyone is ever very concerned about type I error look here (although you should probably be more worried about type II error) 
...
2014: I had 58% assignment rate (118/203), similar to Nick's (61% 117/191). *bug here, includes precocial males, fixed in analysis notebook, use results there for the final report  
...
# Fitness vs Data Quality

Throughout any genotyping process there's a tradeoff between accuracy and completeness. This has been especially challenging with microsatellites. There has been a desire to tolerate higher missing data than the previous dataset (very little missing data) because I might score ambiguous alleles differently than previous workers. This has caused a new issue though: as the missing data rate goes up, non-exclusion power goes down. We will assess this more directly later, but for now, do we see a positive relationship between fitness and missingness - an indication of a problem, or is there an inverse relationship - our expectation if everything is working as expected. 

Ultimately all of the attention paid to NEPs and EFPs is important, but this (TLF ~ missingness) is the best indicator that we have a mjaor problem. Since we are mostly focused on parent level results (e.g. TLF, CRR), if parents have spuriously assigned offspring due to their own missingness, this would strongly effect the results.

```{r, warning=FALSE, message=FALSE}
# copied code from the analysis notebook here, which will cause some major issues, if you don't remember to clear your environmnent before starting that notebook! don't use the environment here for that notebook!
...
Okay, looks good! individuals with lots of missing data have lower fitness on average than individuals with complete data. Let's do a better job with the model fit though.

```{r, warning=FALSE, message=FALSE}
require(MASS)
require(effects)
# here we know that TLF is best fit using the negative binomial distribution and log-link
miss_gt_model <- glm.nb(tlf ~ miss_gt, data = parents) 
summary(miss_gt_model)
effects::allEffects(miss_gt_model)
```

Yes, higher missingness is associated with lower TLF, suggesting that type I error due to too low of a cutoff for the number of comapred loci (7) is not likely a problem. This makes sense when you think about how the Cervus assignment works. If a given parent has high missingness, the NEP is expected to be high, but two things act to prevent it from gobbling up all the offspring assignments: (1) Each offspring is compared to all parents, (i.e. we choose the best parent for each offspring), (2) if the true parent was sampled, the LOD will be lower for the spurious parent-offspring duo because it is based on less information

Update here: Did a great deal of work examining the impact of missingness on type I and type II error. Calculated cumulative non-exclusion probabilities across range of data missingness and mismatch levels, as well as number of expected false parentages in the pedigree, given the genetic dataset. All these fears are allayed, here's a summary from that work:

>We expect about 23 spurious assignments given the level of missingness in the data. There are 1,669 assigments in the pedigree, so a little over 1% of the pedigree suffers from type I error, even while making some very conservative assumptions about NEPs. Not bad!
In the end, I think the justification used in previous repoorts is also sufficient here: among the individuals with the worst genotyping, the assignments are likely to contain quite a few spurious parent-offspring pairs, but they represent such a small proportion of our total dataset as to be unimportant to the final pedigree. 
